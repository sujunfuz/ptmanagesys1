<template>
  <ContentWrap>
    <div class="font-size-20px color-colorBlack flex-b items-center">
      <div class="line-height-30px h-30px">
        {{ allData?.auditStatusStr }}{{ t('loanAudit.name') }}</div
      >
      <ElButton type="primary" @click="back">{{ t('common.back') }}</ElButton>
    </div>

    <div class="mt-20px" v-if="allData?.auditStatus == 'WAIT_AUDIT'">
      <el-button type="primary" v-hasPermi="'settlement:finSettlement:audit'" @click="passAudit">{{
        t('AuditCenter.AuditList.sub2')
      }}</el-button>
      <el-button
        type="primary"
        v-hasPermi="'settlement:finSettlement:audit'"
  ...
  <div class="flex-b items-stretch mt-20px">
    ...
    <div class="customW"
      ><SettlementInfo :allData="allData" :period="period" :interestRate="interestRate"
    /></div>
  </div>
...
import { settleDetailApi, auditApi } from '@/api/loanAudit/index'
const { t } = useI18n()
...
const getData = async () => {
  ...
  period.value = JSON.parse(res.data.creditInfo).period
  interestRate.value = JSON.parse(res.data.creditInfo).interestRate
}
