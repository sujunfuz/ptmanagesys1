<template>
  <ContentWrap v-hasPermi="'safetyCenter:safMerBehavior:list'">
  ...
  <ContentWrap class="mt-20px flex-1" :isBlock="false">
    <div class="p-20px flex-c">
      <span class="subString"
        ...
        {{ t('abnormalMerchant.danwei') }}</span
      >
      <div
        v-hasPermi="'safetyCenter:safMerBehavior:exportExcel'"
        class="aCustomBtn"
        @click="exportFile"
...
import {
  ...
  getUserDetailApi,
  safIpAddApi
} from '@/api/safetyCenter/abnormalMerchant'
...
const searchSchema = reactive<FormSchema[]>([
  {
    ...
    componentProps: {
      slots: {
        prepend: () => <div>{t('abnormalMerchant.id')}</div>
      }
    }
  },
  {
    ...
    componentProps: {
      options: status,
      slots: {
        prefix: () => <div>{t('abnormalMerchant.status')}</div>
      }
    }
  },
  {
    ...
    componentProps: {
      slots: {
        prefix: () => <div>{t('abnormalMerchant.sub1')}</div>
      },
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  },
  {
    ...
    formatter: (data: any) => {
      ...
      return <ElTag type={status == 'STATUS_0_0_0_0' ? 'success' : 'danger'}>{statusStr}</ElTag>
    }
  },
  {
          ...
          return (
            <>
              <div class="[&>span]:(cursor-pointer m-2) flex flex-wrap colorMain ">
                ...
                <span
                  v-hasPermi="safetyCenter:safMerBehavior:seal"
                  class="color-red-500"
                  v-show={ row.merchantNo}
                  onClick={() => clickSeal(row)}
                >
                  {t('buyer.action1')}
                </span>
                <span
                  v-hasPermi="safetyCenter:safMerBehavior:ipblack"
                  onClick={() => clickIpBlack(row)}
                >
                  {' '}
                  {t('abnormalMerchant.action1')}{' '}
                </span>
              </div>
            </>
          )
...
const clickSeal = (row: any) => {
  ElMessageBox.confirm(t('abnormalMerchant.sub9'), t('buyer.action1'), {
    confirmButtonText: t('project.confirm'),
    cancelButtonText: t('project.cancel')
  })
    .then(async () => {
      const res = await setUserBanStatusApi({
...
const clickIpBlack = (row: any) => {
  ElMessageBox.confirm(t('abnormalMerchant.sub8'), t('abnormalMerchant.action1'), {
    confirmButtonText: t('project.confirm'),
    cancelButtonText: t('project.cancel')
  })
    .then(async () => {
...
const action = async (row: any, type: string) => {
  ...
  const res = await getUserDetailApi(row.merchantNo)
