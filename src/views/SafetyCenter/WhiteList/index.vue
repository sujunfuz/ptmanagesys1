  ...
  <ContentWrap class="flex-1" :isBlock="false">
    <div class="flex justify-between p-20px">
      <ElRadioGroup v-model="tabPosition" @change="getList()">
        <ElRadioButton v-hasPermi="'safetyCenter:safIp:listWhite'" label="1">{{
          t('whiteList.sub1')
        }}</ElRadioButton>
        <ElRadioButton v-hasPermi="'safetyCenter:safCardGroup:whiteDetail'" label="2">{{
          t('whiteList.sub2')
        }}</ElRadioButton>
        <ElRadioButton v-hasPermi="'safetyCenter:safCard:whiteDetail'" label="3">{{
          t('whiteList.sub3')
        }}</ElRadioButton>
      </ElRadioGroup>
      <ElButton
        v-if="tabPosition == '1'"
        v-hasPermi="'safetyCenter:safIp:addWhite'"
        type="primary"
        @click="AddAction"
      ...
      <ElButton
        ...
        v-hasPermi="'safetyCenter:safCardGroup:addWhite'"
        type="primary"
        @click="AddAction"
        >{{ t('project.add') }}</ElButton
      >
      <ElButton
        ...
        v-hasPermi="'safetyCenter:safCard:addWhite'"
        type="primary"
        @click="AddAction"
        >{{ t('project.add') }}</ElButton
      >
          ...
          return (
            <>
              <div class="[&>span]:(cursor-pointer m-2) flex flex-wrap colorMain ">
                <span
                  v-hasPermi="safetyCenter:safIp:whiteDetail"
                ...
                <span
                  v-hasPermi="safetyCenter:safIp:releaseWhite"
        ...
        default: (data: any) => {
          ...
          return (
            <>
              <div class="[&>span]:(cursor-pointer m-2) flex flex-wrap colorMain ">
                <span
                  v-hasPermi="safetyCenter:safCardGroup:whiteDetail"
                  onClick={() => action(row, 'view')}
                >
                  {' '}
                  {t('project.detail')}{' '}
                ...
                <span
                  v-hasPermi="safetyCenter:safCardGroup:releaseWhite"
                  onClick={() => action(row, 'action')}
                >
                  {t('whiteList.action')}
                </span>
        ...
        default: (data: any) => {
          ...
          return (
            <>
              <div class="[&>span]:(cursor-pointer m-2) flex flex-wrap colorMain ">
                <span
                  v-hasPermi="safetyCenter:safCard:whiteDetail"
                  onClick={() => action(row, 'view')}
                >
                  {' '}
                  {t('project.detail')}{' '}
                ...
                <span
                  v-hasPermi="safetyCenter:safCard:releaseWhite"
                  onClick={() => action(row, 'action')}
                >
                  {t('whiteList.action')}
                </span>
...
const AddAction = () => {
  dialogTitle.value = t('whiteList.sub6')
...
const action = async (row: any, type: string) => {
  switch (type) {
    case 'view':
      ...
      break
    case 'action':
      dialogTitle.value = t('whiteList.action')
      id.value = row.id
      break
  }
  let res
